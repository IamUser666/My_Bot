...
async def process_callback(callback_query: types.CallbackQuery):
    if callback_query.data == "like":  # ← 4 пробела!
        await callback_query.answer("❤️ Лайк! Видео добавлено в избранное.")
    else:
        await callback_query.answer()
    await send_random_video(callback_query.from_user.id)  # ← 4 пробела!
... 
...
async def main():
    http_runner = await web_server()  # Запуск HTTP-сервера
    try:
        await dp.start_polling(bot)
    finally:
        await bot.session.close()
        await http_runner.cleanup()  # Корректное завершение
